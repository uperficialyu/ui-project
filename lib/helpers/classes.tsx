...
//   return function x(name?: any | ClassToggles, options?: Options) {
//     const namesObject = (typeof name === 'string' || name === undefined) ? { [name]: name } : name;
//     const scoped = Object.entries(namesObject).filter(kv => kv[1] !== false).map(kv => kv[0])
//       .map(name => [prefix, name].filter(Boolean).join('-')).join(' ')
